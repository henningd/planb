...
class SeverityIndicator
{
    ...
    public static function auditActionIcon(string $action): string
    {
        ...
        return match (true) {
            ...
            $action === 'member.invited' => 'envelope',
            $action === 'member.joined' => 'user-plus',
            $action === 'member.role_changed' => 'identification',
            $action === 'member.removed' => 'user-minus',
            $action === 'member.disabled' => 'pause-circle',
            $action === 'member.reactivated' => 'arrow-path',
            $action === 'security.password_changed' => 'key',
            $action === 'security.2fa_enabled' => 'lock-closed',
            $action === 'security.2fa_disabled' => 'lock-open',
            $action === 'security.profile_updated' => 'user-circle',
            $isAssignment => 'link',
